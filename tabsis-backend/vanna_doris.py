...
class VannaDoris(VannaBase):
    ...
    def __init__(self, doris_client: DorisClient, config: Optional[Dict[str, Any]] = None, database: Optional[str] = None):
        """
    ...
    def run_sql(self, sql: str) -> Any:
        ...
        try:
            ...
            results = self.doris_client.execute_query(sql, database=self.database)
    ...
    def get_table_names(self) -> List[str]:
        ...
        """
        try:
            tables = self.doris_client.get_tables(database=self.database)
            return tables
        except Exception as e:
    ...
    def get_table_schema(self, table_name: str) -> List[Dict[str, Any]]:
        ...
        """
        try:
            schema = self.doris_client.get_table_schema(table_name, database=self.database)
            return schema
        except Exception as e:
    ...
    def get_sql_prompt(self, question: str, question_sql_list: List[Dict[str, str]],
        ...
        prompt += "**ORDER BY RULES** (CRITICAL FOR CORRECT SORTING):\n"
        prompt += "- ❌ NEVER use Chinese aliases in ORDER BY: ORDER BY 总额 DESC (WRONG - causes string sorting)\n"
        prompt += "- ✅ ALWAYS use the aggregate function directly: ORDER BY SUM(amount) DESC (CORRECT)\n"
        prompt += "- ✅ OR use column position number: ORDER BY 2 DESC (if SUM(amount) is 2nd column)\n"
        prompt += "- This ensures NUMERIC sorting instead of STRING sorting\n\n"

        prompt += "**DATA CLEANING RULES** (CRITICAL FOR CORRECT GROUPING):\n"
        prompt += "- ⚠️ Text fields often contain hidden spaces, newlines (\\n, \\r), or tabs\n"
        prompt += "- ✅ ALWAYS use TRIM() when grouping by text fields (names, companies, locations)\n"
        prompt += "- ✅ In SELECT: SELECT TRIM(buyer_name) AS 公司 (ensures clean output)\n"
        prompt += "- ✅ In GROUP BY: GROUP BY TRIM(buyer_name) (prevents duplicate companies)\n"
        prompt += "- Example: '广州科纳公司' and '广州科纳公司 ' (with space) should be treated as ONE company\n\n"
        ...
        prompt += "- Question: '销售额最多的前10个客户' → SELECT TRIM(buyer_name) AS 公司, SUM(amount) AS 总额 ... GROUP BY TRIM(buyer_name) ORDER BY SUM(amount) DESC LIMIT 10\n\n"
...
class VannaDorisOpenAI(VannaDoris):
    ...
    def __init__(self, doris_client: DorisClient,
        ...
        VannaDoris.__init__(self, doris_client=doris_client, config=config, database=database)

        # Set API configuration
        self.api_key = api_key
        self.model = model or 'deepseek-chat'
        self.base_url = _normalize_base_url(base_url) or 'https://api.deepseek.com'
